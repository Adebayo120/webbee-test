...
export class Initial1687115740892 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`bookable_calender\` (\`id\` int NOT NULL AUTO_INCREMENT, \`day\` int UNSIGNED NOT NULL, \`opening_hour_in_minutes\` int UNSIGNED NULL, \`closing_hour_in_minutes\` int UNSIGNED NULL, \`available\` tinyint NOT NULL DEFAULT 1, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`serviceId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`configured_break\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`start_hour_in_minutes\` int UNSIGNED NOT NULL, \`end_hour_in_minutes\` int UNSIGNED NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`serviceId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`service\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`bookable_duration_in_minutes\` int UNSIGNED NOT NULL, \`break_between_slots_in_minutes\` int UNSIGNED NOT NULL DEFAULT '0', \`future_bookable_days\` bigint UNSIGNED NOT NULL DEFAULT '20', \`bookable_appointments_per_slot_count\` bigint UNSIGNED NOT NULL DEFAULT '1', \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`businessAdministratorId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`bookable_calender\` ADD CONSTRAINT \`FK_01dda61bc5bee564a8358d38b52\` FOREIGN KEY (\`serviceId\`) REFERENCES \`service\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`configured_break\` ADD CONSTRAINT \`FK_20117f91dd4cbaec7e0d56423a3\` FOREIGN KEY (\`serviceId\`) REFERENCES \`service\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`planned_off\` ADD CONSTRAINT \`FK_a50a3320d6429146b1d99281f4f\` FOREIGN KEY (\`serviceId\`) REFERENCES \`service\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`service\` ADD CONSTRAINT \`FK_2869188df5a90d5bb0d9ffc26e7\` FOREIGN KEY (\`businessAdministratorId\`) REFERENCES \`business_administrator\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
